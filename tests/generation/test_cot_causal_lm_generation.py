...
from confidentllm.generation.cot_causal_lm_generation import (
...
def test_model_not_set_error(mock_tokenizer: PreTrainedTokenizer) -> None:  # noqa: F811 - pytest fixtures
    ...
    method: CausalLMGenerationMethod = CoTDecodingCausalLMGenerationMethod(
...
def test_generate(
    ...
    method = CoTDecodingCausalLMGenerationMethod(
        confidence_extraction_method=PredictiveProbability(),
    ...
    if (generation_output.generated_ids != MOCK_SEQUENCE_MULTI_BEAM).any():  # type: ignore[attrtibute]
        msg = (
